...
{
    dE_stream_t <- dE_cum_amnt_weighted(
        LRD_stream[1:t, ])
    dE_rain_t <- dE_cum_amnt_weighted(
        LRD_rain[1:t, ])
    p_d18O_t <- p_iso(
...
LRD_new_for_plot <- LRD_new %>%
    select(date_time:d2H_p_t) %>%
    gather(
        key = isotope_dE_stream_t, value = dE_stream_t,
        d18O_stream_t:d2H_stream_t) %>%
    mutate(
        dE_stream_isotope = as_vector(map(
            str_split(isotope_dE_stream_t, "_"),
            `[[`,
            1))) %>%
    select(-isotope_dE_stream_t) %>%
    gather(
        key = isotope_dE_rain_t, value = dE_rain_t,
        d18O_rain_t:d2H_rain_t) %>%
    mutate(
        dE_rain_isotope = as_vector(map(
            str_split(isotope_dE_rain_t, "_"),
            `[[`,
            1))) %>%
    select(-isotope_dE_rain_t) %>%
    gather(
        key = source, value = dE_t,
        dE_stream_t, dE_rain_t) %>%
    mutate(
        source = ifelse(str_detect(source, "stream"),
            "stream", "rain")) %>%
    mutate(
        isotope = ifelse(source == "stream",
            dE_stream_isotope, dE_rain_isotope)) %>%
    select(-dE_stream_isotope, -dE_rain_isotope) %>%
    gather(
        key = p_isotope, value = p_t,
        d18O_p_t, d2H_p_t) %>%
    mutate(
        p_isotope = as_vector(map(
            str_split(p_isotope, "_"),
            `[[`,
            1))) %>%
...
my_palette <- c(
    ...
    "#983b33",  # dark red
    "#44782b")  # dark green
fig_4 <- ggplot(LRD_new_for_plot, aes(date_time, dE_t, col = paste(source, isotope))) +
...
my_palette <- c(
    "#b73f41",  # red
    "#599f37")  # green
fig_5 <- ggplot(LRD_new_for_plot, aes(date_time, p_t * 100, col = isotope)) +
    geom_point() +
...
tiff("fig-4.tiff", width = 15, height = 10, units = "cm", res = 300)
fig_4
dev.off()

tiff("fig-5.tiff", width = 15, height = 10, units = "cm", res = 300)
fig_5
dev.off()
