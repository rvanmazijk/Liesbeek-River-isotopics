...
make_panel_a <- function() {
    ...
    base_plot <- ggplot(rain_river_spread) +
        geom_smooth(
            ...
            col = "#55a0c7",
            size = 0.7) +
        geom_point(aes(Rain, River), col = "#55a0c7") +
    ...
    annotated_plot <- base_plot +
        annotate(
            ...
            label = paste0(
                "italic(R)^{2} == ",
                format(
                    summary(fit_stream_vs_rain)$r.squared,
                    digits = 3)),
            x = 12, y = 1500,
            hjust = 1,
            col = "#0f4969",
...
make_panel_bc <- function(isotope = c("d2H", "d18O")) {
    ...
    base_plot <-
        ...
        geom_smooth(
            ...
            size = 0.7) +
        xlab(switch(isotope,
            "d2H" = expression(paste(
                delta^{2}, "H"["Rain" ], "(‰)")),
            "d18O" = expression(paste(
                delta^{18}, "O"["Rain" ], "(‰)")))) +
        ylab(switch(isotope,
            "d2H" = expression(paste(
                delta^{2}, "H"["Stream" ], "(‰)")),
        ...
        annotate(
            ...
            label = switch(isotope,
                "d2H" = paste0(
                    "italic(R)^{2} == ",
                     format(
                         summary(fit_d2H)$r.squared,
                         digits = 3)),
                "d18O" = paste0(
                     "italic(R)^{2} == ",
                     format(
                         summary(fit_d18O)$r.squared,
                         digits = 3))),
            parse = TRUE,
            hjust = 1,
            x = switch(isotope,
        ...
        annotate(
            ...
            label = switch(isotope,
                "d2H" = paste0(
                    "Slope == ",
                    format(
                        summary(fit_d2H)$coefficients[2],
                        digits = 3)),
                "d18O" = paste0(
                    "Slope == ",
                    format(
                        summary(fit_d18O)$coefficients[2],
                        digits = 3))),
            parse = TRUE,
            hjust = 1,
            x = switch(isotope,
...
tiff("fig-2.tiff", width = 10, height = 20, units = "cm", res = 300)
fig_2
dev.off()
